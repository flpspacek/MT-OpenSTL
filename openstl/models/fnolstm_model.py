...
class FNOLSTM_B_Model(nn.Module):
        def __init__(self, in_T: int, in_channels: int, num_hidden: int, n_layers: int, fno_block_args: dict, **kwargs) -> None:
            super(FNOLSTM_B_Model, self).__init__()
            self.in_T = in_T
            self.in_chanels = in_channels
            self.n_layers = n_layers
            self.cells = nn.ModuleList([FNOLSTMCell(fno_block_args, in_channels=(in_channels if n == 0 else num_hidden), num_hidden=num_hidden) for n in range(n_layers)])
