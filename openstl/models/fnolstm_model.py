...
from openstl.modules.fnolstm_modules import FNOLSTMCell


class FNOLSTM_B_Model(nn.Module):
        def __init__(self, in_T: int, in_channels: int, num_hidden: int, fno_block_args: dict, **kwargs) -> None:
            super(FNOLSTM_B_Model, self).__init__()
            self.in_T = in_T
            self.in_chanels = in_channels
            self.fnolstm = FNOLSTMCell(fno_block_args, in_channels=in_channels, num_hidden=num_hidden)
        ...
        def forward(self, x, teacher_forcing_prob: float=0.5,) -> torch.Tensor:
            ...
            for i in range(self.in_T - 1):
                x_next = x[:, :, i, :, :]
                c_t, h_t = self.fnolstm(x_next, c_t, h_t) 
            ...
            for i in range(total_T - self.in_T):
                ...
                c_t, h_t = self.fnolstm(last_frame, c_t, h_t) 
